...
const generateTShirtPatternWithStyleFlow = ai.defineFlow(
  ...
  async input => {
    let textPrompt = `Generate a clean, clear, and creative image suitable for printing on a variety of products, optimized for web and mobile use. The concept is: '${input.prompt}'`;
    if (input.style) {
        textPrompt += `, in the style of ${input.style}`;
    }
    textPrompt += `. The image should be detailed and sharp, around 1024x1024 pixels.
