    ...
    <div className="relative h-full w-full bg-background animate-fade-in" {...swipeHandlers}>
      {generatedPattern && (
          <div className="w-full h-full flex items-center justify-center p-8 animate-scale-in">
            <Image src={generatedPattern} alt="生成的创意图案" width={500} height={500} className="max-w-full max-h-full object-contain rounded-lg shadow-2xl shadow-accent/20" />
          </div>
      )}
      <div className="absolute top-0 left-0 right-0 p-4 flex items-center bg-gradient-to-b from-black/70 to-transparent animate-fade-in-up" style={{ animationDelay: '100ms' }}>
        ...
        <h2 className="text-xl font-bold mx-auto text-white" style={{ textShadow: '1px 1px 3px rgba(0,0,0,0.7)' }}>第一步：创意图案</h2>
        <div className="w-9 h-9"></div>
      </div>
      <HistoryNavigator currentIndex={historyIndex} total={totalHistory} onNavigate={onNavigate} />
      <div className="absolute bottom-0 left-0 right-0 p-6 pt-12 bg-gradient-to-t from-black/80 to-transparent text-center animate-fade-in-up" style={{ animationDelay: '300ms' }}>
        {isModelGenerating ? (
          <div className="flex items-center justify-center mt-2 text-gray-300 h-11">
            <Loader2 className="animate-spin mr-2" size={16} /> <span>正在为您生成模特试穿图...</span>
          </div>
        ) : (
          <Button onClick={onGoToModel} className="w-full bg-primary/80 hover:bg-primary backdrop-blur-sm text-primary-foreground font-bold py-3 px-4 rounded-full flex items-center justify-center transition-all duration-300 transform hover:scale-105 active:scale-100 h-11">
