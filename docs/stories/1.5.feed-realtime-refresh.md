...
## Tasks / Subtasks
- [ ] Client refresh hook (AC: 1,2)  
  - [ ] Add polling mechanism (e.g., React hook or util) that triggers feed refetch every configurable interval, with exponential backoff on failure.  
  - [ ] Update `FeedScreen` to prepend new entries while maintaining existing list state.  
- [ ] Server endpoint support (AC: 3)  
  - [ ] Extend feed service to accept “since” timestamps or cursors for incremental loads.  
  - [ ] Ensure fallback path returns complete page when incremental data unavailable.  
- [ ] Observability (AC: 4)  
  - [ ] Emit metrics (`feed.refresh.frequency`, `feed.refresh.newItems`) and log user interactions triggered by refresh.  
  - [ ] Document dashboards/alerts for refresh staleness and failures.  
- [ ] Feature flag & compatibility (AC: 1,3,5)  
  - [ ] Wrap new refresh behavior behind Remote Config/env flag; confirm `/` legacy experience unaffected.  
  - [ ] Provide rollback instructions (disable flag, revert to manual refresh).  
- [ ] Testing (AC: 5)  
  - [ ] Jest tests for polling utilities and service incremental endpoint.  
  - [ ] Playwright integration test verifying new entries appear without disrupting pagination.  
  - [ ] Manual regression confirming legacy experience unchanged when flag off.
...
## Testing
- Jest unit tests for polling/refresh logic.  `[Source: architecture.md#8-testing-strategy]`
- Playwright integration test verifying refresh behavior on `/beta`.  `[Source: architecture.md#8-testing-strategy]`
- Manual regression ensuring legacy `/` unaffected.  `[Source: architecture.md#73-deployment-strategy]`
